...
ggplot(state_case_percap, aes(x = date)) +
  geom_col(aes(y = new_cases_percap), fill = "pink", col = NA) +  # Corrected placement of `fill` and `col`
  geom_line(aes(y = roll), col = "darkred", size = 1) +
  theme_linedraw() +
  facet_wrap(~state, nrow = 2, scales = "free_y") +  # Corrected `rnow` to `nrow`
...
### Q6 Response
After scaling the covid case data to a per capita basis, the states looked much more similar, eliminating some of the bias against larger states.
